...
class Trainer:
    def __init__(
        ...
        **kwargs,
        ...
        # Initialize training logs for batch-level tracking
        self.train_logs = []
    ...
    # NOTE: ignore this. just used for reporting. 
    ...
    def run_loop(self):
        ...
        for epoch in range(start_epoch, self.steps):
            ...
            for batch in pbar:
                ...
                batch_total_loss = batch_dloss.item() + batch_closs.item()
                self.train_logs.append({
                    'epoch': epoch + 1,
                    'batch': self.batch_counter,
                    'discrete_loss': batch_dloss.item(),
                    'continuous_loss': batch_closs.item(),
                    'total_loss': batch_total_loss,
                    'learning_rate': curr_lr,
                    'closs_weight': closs_weight,
                    'dloss_weight': dloss_weight,
                    'is_best_model': False,  # Will be updated when best model is saved
                    'is_best_ema_model': False
                })
            ...
            if total_loss < best_loss:
                ...
                # Mark the last batch of this epoch as best model
                if self.train_logs:
                    self.train_logs[-1]['is_best_model'] = True
            ...
            if ema_total_loss < best_ema_loss and self.curr_epoch > 4000:
                ...
                # Mark the last batch of this epoch as best EMA model
                if self.train_logs:
                    self.train_logs[-1]['is_best_ema_model'] = True
        ...
        if self.train_logs:
            ...
            train_logs_path = os.path.join(self.model_save_path, "train_logs.csv")
            train_logs_df.to_csv(train_logs_path, index=False)
    ...
    def evaluate_generation(
        ...
        # HACK: i added this. we don't want the plots here.
        plot_density = False
        if plot_density:
